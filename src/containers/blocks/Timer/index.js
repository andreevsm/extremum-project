...
const updateClock = (setCurrentTime) => {
  const timer = new Date(Date.parse(new Date('2019-07-08')) + 15 * 24 * 60 * 60 * 1000);
  const time = getTimeRemaining(timer);
  setCurrentTime(`${time.days}:${(`0${time.hours}`).slice(-2)}:${(`0${time.minutes}`).slice(-2)}:${(`0${time.seconds}`).slice(-2)}`);
};
